(* XXX(dinosaure): design & implementation
   ...
   - A DNS resolver can respond to queries from UDPv4, TCPv4 and possibly TLS
     (if a TLS configuration is offered).
   - A DNS resolver can send queries and wait for responses from DNS servers via
     UDPv4, TCPv4 or TLS (if the [opportunistic] option is given)

   Thus, a DNS resolver acts as a server on one side but also as a client on the
   other side with other DNS servers.
   ...
   notable difference between UDPv4 and TCPv4 and/or TLS communication.

   With regard to UDPv4 communication, the server waits for queries and our
   [Dns_resolver.t] state will respond to these queries as soon as it has the
   opportunity. This state is responsible for correctly linking the responses to
   the queries (in particular by the DNS packet ID).

   These queries may require our resolver to also send these queries to other
   DNS servers. In this case, we send these queries to the servers and then wait
   for a response, which we then send to our [Dns_resolver.t] state.
   [handle_query3] implements querying an external DNS server and transferring
   the response to our state.

   With regard to TCPv4/TLS, the process is a little more complex. This is
   because the cost of initiating a TCPv4/TLS connection is not insignificant.
   If we need to send a query to a DNS server via TCPv4/TLS, we can possibly
   reuse an already initiated connection instead of initiating a new one. Thus,
   the connection to a DNS server via TCPv4/TLS is managed by the
   [new_connection] function (with an abstraction on TCPv4/TLS). This task has
   two subtasks: a [reader] that does not exceed 2 seconds and a packet [writer]
   according to a queue.

   The objective is to be able to take the opportunity to send packets "at the
   same time" while attempting to read several packets from the external DNS
   server (still within the 2-second limit). In this way, sending a query to an
   external DNS server first involves searching for an active connection with
   that server. If one is found, an attempt is made to notify the task, using
   [Qout.t], of the sending of a new query. At the same time, DNS packets are
   read from this external server and the task ends after 2 seconds. This task
   of reading/writing to an external DNS server is implemented in the
   [new_connection] function.

   The attempt to search for an already active connection with the external DNS
   server is implemented in the [handle_query0] function.

   If the query could not be sent (because the 2 seconds elapsed before anything
   could be written), an attempt is made to create a new TCPv4 connection with
   the external DNS server (this attempt is implemented in [handle_query2]).
   Finally, if the query still cannot be sent, a last attempt is made by sending
   the query via UDPv4 ([handle_query3]).

   In other words, if a query must be sent via TCPv4 (without [opportunistic]
   ...
   - [handle_query3]: if the TCPv4 connection is impossible, we try via UDPv4

   In the case of the TCPv4/TLS server, a task is used to retrieve new clients
   ...
   clients who asked us the questions or directly via a simple UDPv4 packet. *)
...
type t = {
  ...
  ; condition: Miou.Condition.t
  ; queue: pktin Queue.t
  ; tcpv4: Mnet.TCPv4.state
  ; udpv4: Mnet.UDPv4.state
...
module A = struct
  ...
  let connect t (dst, port) =
    let[@warning "-8"] (Ipaddr.V4 ipv4) = dst in
    connect t.tcpv4 (ipv4, port)
end
...
module B = struct
  ...
  let peers flow = A.peers (file_descr flow)

  let connect t (dst, port) =
    let fn () =
      let flow = A.connect t (dst, port) in
...
let new_connection : type flow.
 ...
 fun (module Flow) ->
  ...
  fun t qout (dst, port) ->
    ...
    let flow = Flow.connect t (dst, port) in
...
let handle_query3 t (_protocol, dst, data) =
  ...
  let[@warning "-8"] (Ipaddr.V4 dst) = dst in
  let src_port = random_src_port () in
  Log.debug (fun m -> m "query *:%d -> %a:%d" src_port Ipaddr.V4.pp dst 53);
  let* () = Mnet.UDPv4.sendto t.udpv4 ~dst ~src_port ~port:53 data in
  let buf = Bytes.create 1500 in
  let fn () = Mnet.UDPv4.recvfrom t.udpv4 ~port:src_port buf in
...
let handle_query0 t ((protocol, _dst, _data) as elt) =
  ...
  if not sent then
    ...
    | `Udp, false -> handle_query3 t elt

(*/*)

let handle_answer_on_udp t (_protocol, dst, port, _ttl, data) =
  let[@warning "-8"] (Ipaddr.V4 dst) = dst in
  match Mnet.UDPv4.sendto t.udpv4 ~dst ~src_port:53 ~port data with
  | Ok () -> ()
  | Error `Route_not_found ->
      Log.err (fun m ->
          m "Impossible to answer to %a:%d: no route found" Ipaddr.V4.pp dst
            port)
...
let handle_answer t ((protocol, _dst, _port, _ttl, _data) as elt) =
  match protocol with
  | `Udp -> handle_answer_on_udp t elt
  | `Tcp -> handle_answer_on_conn t elt

(*/*)

let rec on_udp t =
  (* TODO(dinosaure): replace [1500] by the MTU of [t.udpv4]. *)
  let buf = Bytes.create 1500 in
  let len, (peer, pport) = Mnet.UDPv4.recvfrom t.udpv4 ~port:53 buf in
  Log.debug (fun m ->
      m "new UDPv4/DNS packet from %a:%d" Ipaddr.V4.pp peer pport);
  let data = Bytes.sub_string buf 0 len in
  let conn = `Udp in
  let dst = Ipaddr.V4 peer in
  let port = pport in
  let mono = Int64.of_int (Mkernel.clock_monotonic ()) in
  let wall = now () in
  let query = true in
...
let on_tcp t =
  let rec go orphans listen =
    clean_up orphans;
    let flow = Mnet.TCPv4.accept t.tcpv4 listen in
    let _, (dst, port) = Mnet.TCPv4.peers flow in
    Log.debug (fun m ->
        m "new TCPv4/DNS connection from %a:%d" Ipaddr.pp dst port);
    let _ =
    ...
    go orphans listen
  in
  go (Miou.orphans ()) (Mnet.TCPv4.listen t.tcpv4 53)
...
let on_tls t cfg =
  let rec go orphans listen =
    ...
    go orphans listen
  in
  go (Miou.orphans ()) (Mnet.TCPv4.listen t.tcpv4 853)
...
let create ?(features = []) ?tls cfg tcpv4 udpv4 primary =
  let now = Fun.compose Int64.of_int Mkernel.clock_monotonic in
  let rng len = Mirage_crypto_rng.generate len in
  let ip_protocol = `Ipv4_only in
  let state = Dns_resolver.create ~ip_protocol features (now ()) rng primary in
